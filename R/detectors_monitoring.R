...
SBQ.mon.infinite <- function(formula, T, alternative = c("two.sided", "greater", "less"), H = NULL){
  alternative=match.arg(alternative)
  n <- dim(model.matrix(formula))[1] #current time point
  k <- dim(model.matrix(formula))[2]
  if (is.null(H)){
  ...
  # critical values and test decision
  if(alternative == "two.sided"){
    # crit.val <- crit.SBQ.Mtest(k,Inf)$crit
    crit.val = NA
  } else {
    # crit.val <- crit.SBQ.Mtest(k, Inf, "one.sided")$crit
    crit.val = NA
  }
  rejection <- statistic > crit.val
  # detection time point
