...
## for the Q M-test with finite monitoring horizon
## ####################################################################
## ####################################################################
rm(list=ls())
start<-Sys.time()
...
## ##################################
sim.Q = function(T,k,m){
  M = (tail(m,1)-1)*T
  BrownianMotion = function(T)  ( cumsum(c(rnorm(M,0,sqrt(1/T)))) )
  detector.dim = function(j){
    W = BrownianMotion(T)
    cummax(abs(W)/(1 + 2*(1:M)/T))
  }
  Qcummax = apply(sapply(1:k,detector.dim),1,cummax)
  Qcummax[,ceiling((m-1)*T)]
}
##
out = parSapply(cl,rep(T,MC),sim.Q, k=k, m=m,simplify="array")
