...
## Load simulated data
## ##################################
load(file = "figure3_data_upper.RData")
## ##################################
## Figure 3
## ##################################
pdf('figure3.pdf', width=30, height=22.5, pointsize = 30 )
# par(oma = c(1, 0, 0, 0))
par(mfrow=c(3,3))
c.range <- 0:30
##
...
##
rstar.range <- 1:39/40
plot(rstar.range, powcur.ret.c10[1,], type='l', col=1, main=expression(paste("break size: c/",sigma," = 10")), ylab = '', xlab = '', lty=1, lwd=4, ylim = c(0.05,1), cex.lab = 1.2, cex.axis=1.2, cex.main = 2)
mtext(text = "rejection frequency",side = 2, line = 2.4, cex=0.8)
...
##
load(file = "figure3_data_lower.RData")
c.range <- 5:30
##
...
plot(rstar.range, delcur.mon.c20[1,], type='l', col=1, main=expression(paste("break size: c/",sigma," = 20")), ylab = '', xlab = '', lty=3, lwd=4, ylim = c(0,0.15), cex.lab = 1.2, cex.axis=1.2, cex.main = 2)
mtext(text = expression(paste(tau, "*")),side = 1, line = 2.2, cex=0.9)
mtext(text = 'relative mean delay', side = 2, line = 2.4, cex=0.8)
