...
getplot.newcases <- function(modeldata, name, startdate = "2020-03-10", enddate = NULL){
  ...
  abline(v=modeldata$Q.dettime, lwd=2, lty = 3)
...
getplot.detectors <- function(modeldata, name, enddate = NULL){
  ...
  endindex <- which(mon.dates == enddate)
  plot(mon.dates[1:endindex], plotdata1[1:endindex], type='l', ylim = c(-0.55, 1.7), main=paste("Scaled detectors", name), xlab="", ylab="",cex.lab = 1.2, cex.axis=1.2, cex.main = 1.5)
...
pdf(paste("figure5.pdf", sep=""), width=30, height=37.5, pointsize = 30)
par(mfrow=c(5,3))
getplot.newcases(US.modeldata, "US total", enddate = enddate)
getplot.recresid(US.modeldata, "US total", enddate = "2020-06-24")
getplot.detectors(US.modeldata, "US total", enddate = "2020-06-24")
getplot.newcases(AZ.modeldata, "Arizona", enddate = enddate)
getplot.recresid(AZ.modeldata, "Arizona", enddate = "2020-06-03")
getplot.detectors(AZ.modeldata, "Arizona", enddate = "2020-06-03")
getplot.newcases(FL.modeldata, "Florida", enddate = enddate)
getplot.recresid(FL.modeldata, "Florida", enddate = "2020-06-11")
getplot.detectors(FL.modeldata, "Florida", enddate = "2020-06-11")
getplot.newcases(NV.modeldata, "Nevada", enddate = enddate)
getplot.recresid(NV.modeldata, "Nevada", enddate = "2020-06-11")
getplot.detectors(NV.modeldata, "Nevada", enddate = "2020-06-11")
getplot.newcases(TX.modeldata, "Texas", enddate = enddate)
getplot.recresid(TX.modeldata, "Texas", enddate = "2020-06-05")
getplot.detectors(TX.modeldata, "Texas", enddate = "2020-06-05")
dev.off()
